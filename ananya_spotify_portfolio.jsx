...
export default function AnanyaSpotifyPortfolio() {
  ...
  const favoriteSongs = [
    {
      ...
      cover: "https://placehold.co/300x300/0b0b0b/ffffff?text=GO+BACK",
    },
    {
      rank: 2,
      title: "Experience",
      artist: "Ludovico Einaudi",
      album: "Einaudi: Experience",
      cover: "https://placehold.co/300x300/171717/ffffff?text=EXPERIENCE",
    },
    {
      rank: 3,
      title: "White Keys",
      ...
      cover: "https://placehold.co/300x300/1a1a1a/ffffff?text=WHITE+KEYS",
    },
    {
      rank: 4,
    ...
    },
    {
      ...
      artist: "Noah Kahan",
      album: "Porch Light",
      cover: "https://placehold.co/300x300/111111/ffffff?text=PORCH+LIGHT",
    },
    {
      rank: 6,
      ...
      artist: "SYML",
      album: "SYML",
      cover: "https://placehold.co/300x300/141414/ffffff?text=WHERE'S+MY+LOVE",
    },
  ];
  ...
  const styles = `
    ...
    .layout-body { display: flex; gap: 8px; height: 100vh; padding: 8px; padding-bottom: calc(var(--footer-height) + 24px); }
    ...
    .home-link-right span { color:var(--text-primary); font-size:30px; font-weight:700; letter-spacing:-0.03em; }
    ...
    .library-header-left span { font-size:30px; font-weight:700; }
    ...
    .library-toolbar span { color:var(--text-secondary); font-size:20px; }
    ...
    .hero-image { width:260px; height:260px; border-radius:999px; object-fit:cover; background:black; box-shadow: 0 25px 70px rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.1); flex-shrink:0; }
    .hero-cover { width:260px; height:260px; border-radius:18px; background:rgba(0,0,0,0.38); display:flex; align-items:center; justify-content:center; font-size:92px; box-shadow: 0 25px 70px rgba(0,0,0,0.45); border: 1px solid rgba(255,255,255,0.1); flex-shrink:0; }
    .hero-content { display:flex; flex-direction:column; padding-bottom:8px; }
    .verified-badge { display:flex; align-items:center; gap:10px; margin-bottom:16px; font-size:24px; color:rgba(255,255,255,0.9); }
    .hero-title { font-size:110px; line-height:0.95; letter-spacing:-0.07em; font-weight:900; margin:0; max-width:900px; }
    .hero-subtitle { font-size:32px; color:rgba(255,255,255,0.8); margin:20px 0 0; max-width:900px; }
    ...
    .play-button { width:64px; height:64px; border:none; border-radius:999px; background:var(--primary-green-hover); color:black; display:flex; align-items:center; justify-content:center; box-shadow: 0 15px 40px rgba(30,215,96,0.35); cursor:pointer; transition:transform var(--transition-fast), background var(--transition-fast); }
    .play-button:hover { transform:scale(1.03); background:var(--primary-green-hover); }
    .follow-button { border:1px solid rgba(255,255,255,0.3); color:var(--text-primary); background:transparent; border-radius:var(--border-radius-pill); padding:16px 28px; font-size:24px; font-weight:600; cursor:pointer; }
    .featured-section { display:flex; gap:16px; padding:0 40px 32px; }
    .featured-card { flex:1; min-height:116px; border-radius:18px; padding:26px 30px; position:relative; overflow:hidden; border:1px solid rgba(255,255,255,0.05); cursor:pointer; text-align:left; transition:transform var(--transition-normal), background var(--transition-normal); }
    .featured-card:hover { transform:translateY(-1px); }
    .featured-card h3 { margin:0; font-size:22px; font-weight:700; }
    .featured-card p { margin:8px 0 0; font-size:20px; color:rgba(255,255,255,0.65); }
    .featured-projects { background: linear-gradient(135deg, rgba(21,87,52,0.95), rgba(8,17,13,0.95)); }
    .featured-experience { background: linear-gradient(135deg, rgba(95,24,24,0.95), rgba(18,18,18,0.95)); }
    .featured-education { background: linear-gradient(135deg, rgba(18,48,94,0.95), rgba(14,18,28,0.95)); }
    .featured-icon { position:absolute; right:26px; bottom:10px; font-size:58px; opacity:0.2; }
    ...
    .playlist-header-row h2 { margin:0; font-size:44px; font-weight:700; letter-spacing:-0.04em; }
    ...
    .section-title { margin:0; font-size:44px; font-weight:700; letter-spacing:-0.04em; }
    .contest-ranking-btn { border:1px solid rgba(255,255,255,0.3); background:transparent; color:rgba(255,255,255,0.9); border-radius:999px; padding:12px 24px; font-size:18px; font-weight:500; cursor:pointer; }
    .tracks-list { display:flex; flex-direction:column; gap:4px; }
    .track-row { grid-template-columns: 90px minmax(0, 1fr) 220px; }
    .track-index { display:flex; align-items:center; gap:14px; color:#a1a1aa; }
    .track-index-number { width:18px; text-align:right; font-size:22px; }
    ...
    .track-img { width:56px; height:56px; border-radius:6px; object-fit:cover; border:1px solid rgba(255,255,255,0.08); background:#27272a; }
    .track-info-col { min-width:0; display:flex; flex-direction:column; }
    .track-title, .playlist-title { font-size:22px; color:white; font-weight:500; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .track-subtitle, .playlist-subtitle { font-size:20px; color:#a1a1aa; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-top:2px; }
    .track-meta, .playlist-meta { font-size:18px; color:#71717a; text-align:right; }
    .playlist-columns { display:grid; grid-template-columns:50px 1fr 280px; gap:16px; border-bottom:1px solid rgba(255,255,255,0.1); padding:0 16px 12px; margin-bottom:16px; color:#a1a1aa; text-transform:uppercase; letter-spacing:0.08em; font-size:18px; }
    .playlist-row { grid-template-columns:50px 1fr 280px; }
    .playlist-num { font-size:28px; color:#a1a1aa; }
    ...
    .player-now-title { font-size:20px; font-weight:500; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .player-now-sub { font-size:18px; color:var(--text-secondary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
